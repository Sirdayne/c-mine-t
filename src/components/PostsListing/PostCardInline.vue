    ...
    <div class="post-card-inline__content">
      <div class="post-card-inline__header">
        ...
        <div class="post-card-inline__meta">
          ...
          >
            <template v-if="!isWithoutImageTheme">{{ i18n.by }}</template>
            <span>&nbsp;</span>
            <LinkResolver
              ...
              :language-short="languageShort"
              :data-gtm-locator="locatorAuthorName"
            >
      ...
      />
      <div
        v-if="postData.showStats && areStatsShown && languageShort !== 'de'"
...
export default {
  ...
  computed: {
    ...
    prefix() {
      return this.gtmPrefix || this.postData.categoryName?.toLowerCase().replace(/\s/g, '_');
    },
    locatorName() {
      return `${this.prefix}_clickon_${this.postData.id}`;
    },
    locatorAuthorName() {
      if (this.gtmSuffix && this.gtmLocatorType)
        return `${this.prefix}_clickon_${this.gtmLocatorType}_${this.gtmSuffix}`;

      if (this.gtmSuffix) return `${this.prefix}_clickon_${this.postData.id}_${this.gtmSuffix}`;
  ...
  created() {
    ...
    this.areStatsShown =
      this.doViewsExist && !(postData.isPressRelease || postData.isMarketRelease || postData.isExplained);
